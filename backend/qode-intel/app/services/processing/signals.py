...
BULL_WORDS = {
    "buy", "long", "up", "breakout", "bull", "bullish", "rally", "green", "uptrend",
    "nifty", "sensex", "banknifty", "ath", "bo", "reversal", "bounce",
}
BEAR_WORDS = {
    "sell", "short", "down", "breakdown", "bear", "bearish", "fall", "red", "downtrend",
    "dump", "resistance", "rejection", "crash",
}
...
def compute_composite(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df2 = df.with_columns([
        pl.col("content").map_elements(lexical_signal, return_dtype=pl.Float64).alias("lex_signal"),
        pl.struct(["like_count", "retweet_count", "reply_count", "quote_count"])
          .map_elements(engagement_weight, return_dtype=pl.Float64).alias("weight"),
    ])
    ...
    grp = grp.with_columns([
        ...
        (1.96 * (pl.col("std_unweighted") / (pl.col("n") ** 0.5))).alias("ci95"),
    ]).drop(["wsum", "w"]).sort("bucket")
